...
#音を鳴らす装置をワールドに置くか(リピーター3つ(最大遅延2つ、2遅延(クリック一回)1つ)挟んで音符ブロックを置くと表示と合うようになります)下のコマンドの座標を鳴らしたい場所にそれぞれ変更してください(playsoundコマンドが書いてあるところをそれぞれのゲートの待機場所の座標に置き換えてください)
